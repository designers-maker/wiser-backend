...
const Reports = () => {
  ...
  // --- CONFIGURATION ---
  const LOGO_URL = "/images/wiserlogo.png"; 
  ...
  const reportsData = [
      ...
      content: `
        <div class="report-content">
          <div class="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-xl mb-4 border border-blue-100">
            <h2 class="text-xl font-bold text-gray-900 mb-3 text-center">WISER Foundation Annual Impact Report 2024</h2>
          ...
          <div class="mb-6">
            <h3 class="text-lg font-bold text-gray-800 mb-3 pb-2 border-b-2 border-blue-200">Key Metrics</h3>
          ...
          <div class="mb-6">
            <h3 class="text-lg font-bold text-gray-800 mb-3 pb-2 border-b-2 border-indigo-200">Program Distribution</h3>
          ...
          <div class="mb-6">
            <h3 class="text-lg font-bold text-gray-800 mb-3 pb-2 border-b-2 border-blue-200">Geographic Reach</h3>
          ...
          <div class="mb-6">
            <h3 class="text-lg font-bold text-gray-800 mb-3 pb-2 border-b-2 border-green-200">Major Achievements</h3>
          ...
          <div class="mb-6">
            <h3 class="text-lg font-bold text-gray-800 mb-3 pb-2 border-b-2 border-amber-200">Testimonials</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="mb-6">
            <h3 class="text-lg font-bold text-gray-800 mb-3 pb-2 border-b-2 border-red-200">Challenges & Solutions</h3>
            <div class="space-y-4">
    ...
    },
    {
      ...
      content: `
        <div class="report-content">
          <div class="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-xl mb-4 border border-green-100">
            <h2 class="text-xl font-bold text-gray-900 mb-3 text-center">Quarterly Volunteer Statistics & Analysis</h2>
    ...
    },
    {
      ...
      content: `
        <div class="report-content">
          <div class="bg-gradient-to-r from-purple-50 to-violet-50 p-4 rounded-xl mb-4 border border-purple-100">
            <h2 class="text-xl font-bold text-gray-900 mb-3 text-center">CSR Partnership Analysis Report</h2>
            <div class="text-base text-gray-700 leading-relaxed mb-4">
    ...
    },
    {
      ...
      content: `
        <div class="report-content">
          <div class="bg-gradient-to-r from-orange-50 to-amber-50 p-4 rounded-xl mb-4 border border-orange-100">
            <h2 class="text-xl font-bold text-gray-900 mb-3 text-center">Program Effectiveness Analysis Report</h2>
            <div class="text-base text-gray-700 leading-relaxed mb-4">
  ...
  const parseMetrics = (report) => {
    // Return the metrics directly from the report object
    return {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // 1. GENERATE HIGH RESOLUTION CANVAS
      ...
      // Exact calculations to prevent drift
      const pageAvailableHeight = pdfHeight - margin - margin - headerHeight - footerHeight;
      
      // Overlap: 2mm ensures we catch rounded corners, box shadows, and bottom padding
      const overlapMm = 2.0; 
      
      const imgWidth = pdfWidth - (margin * 2);
      
      // Precision conversion: Pixels per mm
      const pxPerMm = canvas.width / imgWidth; 
      
      // Calculate slice size in pixels (with overlap)
      const sliceHeightMm = pageAvailableHeight + overlapMm;
      const sliceHeightPx = sliceHeightMm * pxPerMm;
      
      // Step size in pixels (How much we move down for the next page)
      // We move by the available height, so the overlap fills the gap at the top of the next page
      const stepHeightPx = pageAvailableHeight * pxPerMm;

      // Helper for Header/Footer
      const addHeaderFooter = (pageNum) => {
        const currentDate = new Date().toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' });
        const reportType = getReportTypeLabel(selectedReport.type);
        ...
        pdf.text('WISER', margin, 12);
        
        pdf.setFont('helvetica', 'bold');
        pdf.setFontSize(10);
        pdf.setTextColor(50, 50, 50);
        pdf.text(reportType, margin + 22, 12);
        
        pdf.setFont('helvetica', 'normal');
        pdf.setFontSize(9);
        pdf.setTextColor(100, 100, 100);
        pdf.text(currentDate, pdfWidth - margin - 25, 12);
        ...
        pdf.text(`Page ${pageNum}`, margin, pdfHeight - 10);
        
        pdf.setFont('helvetica', 'bold');
        pdf.setTextColor(37, 99, 235);
        pdf.text('WISER Volunteer', pdfWidth - margin - 25, pdfHeight - 10);
      };
      ...
      while (currentY < canvas.height) {
        ...
        // Calculate exact height for this specific slice
        // If we are near the end, don't go past the canvas height
        let currentSliceHeight = sliceHeightPx;
        if (currentY + currentSliceHeight > canvas.height) {
          currentSliceHeight = canvas.height - currentY;
        }
        ...
        ctx.drawImage(
          canvas, 
          0, currentY, canvas.width, currentSliceHeight, // Source
          0, 0, canvas.width, currentSliceHeight          // Destination
        );

        const sliceImgData = sliceCanvas.toDataURL('image/png');
        const currentSliceHeightMm = currentSliceHeight / pxPerMm;
        ...
        // Move down by the STEP size (not the slice size)
        // This ensures the overlap is consumed on the next page
        currentY += stepHeightPx;
        
        pageNumber++;
      }

      pdf.save(`WISER_Report_${selectedReport.title.replace(/[^a-z0-9]/gi, '_')}.pdf`);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 via-purple-50 to-pink-50 py-8 animate-gradient">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div
          ...
          className="text-center mb-12 bg-gradient-to-r from-blue-600 via-indigo-700 to-purple-600 text-white rounded-2xl p-8 shadow-2xl animate-pulseGlow"
        >
        ...
        >
        <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-4 mb-8 border border-gray-200 hover:shadow-xl transition-all duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {filteredReports.map((report) => (
            <motion.div
              ...
              className="bg-white/90 backdrop-blur-sm rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 cursor-pointer border border-gray-200 overflow-hidden flex flex-col h-full animate-float"
      ...
      {selectedReport && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fadeIn">
          <motion.div
            ...
            className="bg-white rounded-3xl shadow-2xl max-w-6xl w-full max-h-[90vh] h-[85vh] flex flex-col md:flex-row overflow-hidden border border-gray-200"
          >
            
            {/* Sidebar: Charts & Data */}
            <div className="w-full md:w-5/12 bg-gradient-to-b from-indigo-50 via-blue-50 to-purple-50 border-r border-indigo-100 flex flex-col animate-slideInLeft">
              <div className="p-6 border-b border-indigo-200 flex-shrink-0 bg-gradient-to-r from-indigo-100 via-blue-100 to-purple-100">
                <div className="flex justify-between items-center">
                  <h3 className="text-lg font-bold text-indigo-800">Analytics Dashboard</h3>
                  <span className="text-xs font-semibold text-indigo-600 bg-white/50 px-2 py-1 rounded-full animate-pulse">LIVE DATA</span>
                </div>
              </div>
            
              <div className="p-6 overflow-y-auto flex-grow custom-scrollbar bg-gradient-to-b from-white/80 to-indigo-50/30 m-4 rounded-2xl shadow-inner border border-white/50">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {chartData?.totalVolunteers > 0 && (
                      <motion.div
                        ...
                        className="bg-white/80 backdrop-blur-sm p-3 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-all hover:border-blue-200"
                      >
                        <span className="text-xs text-gray-500 font-semibold">Total Volunteers</span>
                        <span className="text-sm font-bold text-indigo-600 block mt-1">{chartData.totalVolunteers.toLocaleString()}</span>
                      </motion.div>
                    )}
                ...
                <div>
                  ...
                  <div className="h-48 w-full bg-white/80 backdrop-blur-sm p-3 rounded-2xl shadow-lg border border-white/50">
            ...
            <div className="w-full md:w-7/12 flex flex-col h-full bg-white relative animate-slideInRight">
              <div className="p-6 border-b border-gray-100 flex-shrink-0 z-10 bg-gradient-to-r from-white to-gray-50">
              ...
              <div className="flex-grow overflow-y-auto custom-scrollbar bg-white relative">
                ...
                >
                  {/* Hidden logo for PDF header */}
                  <div id="pdf-logo" className="hidden">
                    <img src={LOGO_URL} alt="WISER Logo" className="w-16 h-16 object-contain" />
                  </div>
                  ...
                  <div 
                    ...
                    style={{ fontSize: '0.875rem' }} // Reduced font size
                  />
