...
const connectDB = async () => {
  try {
    ...
    console.log('🔗 Database URL: https://wiser-volunteer-default-rtdb.firebaseio.com');
    console.log('🎉 No router/firewall blocking issues!');
  } catch (error) {
    console.error('❌ Firebase connection failed:', error.message);
    console.error('🔧 Please check Firebase configuration and network connectivity');
  }
};
...
app.post('/api/forms/volunteer', async (req, res) => {
  console.log('\n📝 Volunteer form submission received');
  console.log('Data:', req.body);
  try {
    const result = await firebaseDB.volunteer.create(req.body);
    console.log('✅ Volunteer submission saved with ID:', result.id);
    res.status(201).json(result);
  } catch (e) {
    console.error('❌ Error saving volunteer submission:', e.message);
    res.status(400).json({ error: 'Invalid payload', details: e.message });
  }
});

app.post('/api/forms/csr', async (req, res) => {
  console.log('\n📝 CSR form submission received');
  console.log('Data:', req.body);
  try {
    const result = await firebaseDB.csr.create(req.body);
    console.log('✅ CSR submission saved with ID:', result.id);
    res.status(201).json(result);
  } catch (e) {
    console.error('❌ Error saving CSR submission:', e.message);
    res.status(400).json({ error: 'Invalid payload', details: e.message });
  }
});

app.post('/api/forms/college', async (req, res) => {
  console.log('\n📝 College form submission received');
  console.log('Data:', req.body);
  try {
    const result = await firebaseDB.college.create(req.body);
    console.log('✅ College submission saved with ID:', result.id);
    res.status(201).json(result);
  } catch (e) {
    console.error('❌ Error saving college submission:', e.message);
    res.status(400).json({ error: 'Invalid payload', details: e.message });
  }
});
...
app.get('/api/forms/volunteer', async (req, res) => {
  try {
    ...
    const docs = await firebaseDB.volunteer.findAll(limit);
    res.json(docs);
  } catch (e) {
    console.error('❌ Error fetching volunteer submissions:', e.message);
    res.status(500).json({ error: 'Failed to fetch data', details: e.message });
  }
});

app.get('/api/forms/csr', async (req, res) => {
  try {
    const limit = Math.min(parseInt(req.query.limit || '50', 10), 100);
    const docs = await firebaseDB.csr.findAll(limit);
    res.json(docs);
  } catch (e) {
    console.error('❌ Error fetching CSR submissions:', e.message);
    res.status(500).json({ error: 'Failed to fetch data', details: e.message });
  }
});

app.get('/api/forms/college', async (req, res) => {
  try {
    const limit = Math.min(parseInt(req.query.limit || '50', 10), 100);
    const docs = await firebaseDB.college.findAll(limit);
    res.json(docs);
  } catch (e) {
    console.error('❌ Error fetching college submissions:', e.message);
    res.status(500).json({ error: 'Failed to fetch data', details: e.message });
  }
});
