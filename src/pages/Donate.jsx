...
export default function Donate() {
  ...
  async function createPendingDonation(amount, paymentMethod) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errorMsg = result.message || result.error || `Failed to create donation record with status ${response.status}`;
  ...
  async function confirmDonation(donationId) {
    try {
      ...
      if (!response.ok) {
        ...
        const errorMsg = result.message || result.error || `Failed to confirm donation with status ${response.status}`;
        throw new Error(errorMsg);
      }
  ...
  async function handleCopyUri() {
    ...
    try {
      ...
      alert('Unable to track your donation. Please try again or use a direct payment app.');
